...
def measure_lane_curvature(ploty, leftx, rightx):
    ...
    if leftx[0] - leftx[-1] > 60:
        curve_direction = 'Courbe a gauche'
    elif leftx[-1] - leftx[0] > 60:
        curve_direction = 'Courbe a droite'
    else:
        curve_direction = 'Direct'
...
def offCenter(meanPts, inpFrame):
    ...
    direction = "gauche" if deviation < 0 else "droite"

    if abs(deviation) > 0.5:  # Ajuster la tolérance selon les besoins
        status = "Hors des lignes:distance entre le centre de la voiture et le centre des voies>0.5m"
    else:
        status = "Dans les lignes"

    return deviation, direction, status
################################################################################
...
def addText(img, radius, direction, deviation, devDirection, status):

    # Add the radius and center position to the image
    font = cv2.FONT_HERSHEY_TRIPLEX

    if (direction != 'Straight'):
        text = 'Rayon de courbure: ' + '{:04.0f}'.format(radius) + 'm'
        text1 = 'Direction de la courbe: ' + (direction)

    else:
        text = 'Rayon de courbure: ' + 'N/A'
        text1 = 'Direction de la courbe: ' + (direction)
    ...
    # Deviation
    deviation_text = 'Decentre:ajustez ' + str(round(abs(deviation), 3)) + 'm' + ' vers la ' + devDirection
    cv2.putText(img, deviation_text, (50, 200), cv2.FONT_HERSHEY_TRIPLEX, 0.8, (0, 0, 255), 2, cv2.LINE_AA)
    cv2.putText(img, status, (50, 250), font, 0.8, (0, 0, 255), 2, cv2.LINE_AA)
    return img
...
while True:
    ...
    deviation, directionDev, status = offCenter(meanPts, frame)


    # Adding text to our final image
    finalImg = addText(result, curveRad, curveDir, deviation, directionDev, status)
